...
using t_plugin_name = strong::type<
    std::string_view,
    struct s_plugin_name_tag,
    strong::formattable,
    strong::hashable,
    strong::indexed<>,
    strong::ostreamable,
...
inline constexpr luyamlman::types::t_plugin_name
operator"" _esp(
    const char* a_plugin_name,
    size_t      a_length
) noexcept
{
    return luyamlman::types::t_plugin_name{
        std::string_view{ a_plugin_name, a_length }
    };
}
} // namespace luyamlman::literals

#ifdef LUYALMMAN_INSTANTIATE_TEMPLATES_TYPES_TPLUGINNAME
