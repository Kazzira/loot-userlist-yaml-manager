...
template <typename T, typename ATag = struct s_default>
    ...
    public:
        ...
        )
        {
            ...
            m_start = static_cast<T*>(
                memory::arena_allocator::get_instance( a_size ).allocate(
                    a_size * sizeof( T )
                )
            );
        ...
        {
            // No-op for linear allocator, as it does not support deallocation.
